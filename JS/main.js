...
// Theme modes for random background color on todo add
const themesModes = [
  "standard",
  "light",
  "darker",
...
function addToDo(event) {
  ...
  // Change page background to random theme mode when adding a todo
  const randomTheme =
    themesModes[Math.floor(Math.random() * themesModes.length)];
...
function deletecheck(event) {
  ...
  if (item.classList[0] === "delete-btn") {
    // Change background to red gradient when deleting
    changeTheme("red");
  ...
  if (item.classList[0] === "check-btn") {
    ...
    // Change background to green gradient when completing
    if (isCompleting) {
      changeTheme("green");
    }
